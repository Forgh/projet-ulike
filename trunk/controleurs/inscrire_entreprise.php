<?php
	...
	$megacondition = ($nb == 0 and empty($_POST['pseudo'])==false  and empty($_POST['passwd'])==false and empty($_POST['passwdconfirm'])==false and $_POST['passwd'] == $_POST['passwdconfirm'] and empty($_POST['nom_gerant'])==false and empty($_POST['mail_ent'])==false and empty($_POST['adr_ent'])==false and empty($_POST['code_ent'])==false and empty($_POST['pays_ent'])==false);// and (preg_match("(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$",$_POST['passwd_ent'] ));
